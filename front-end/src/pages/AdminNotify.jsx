            ...
            try {
                ...
                const mapped = leaves.flatMap((lv) => {
                    ...
                    const base = {
                        ...
                        message:
                            isAdmin
                                ? `(${lv.studentId || lv.name}) → ${lv.leaveType || "leave"} (${range}) is ${status.toLowerCase()}${status === "Rejected" && lv.adminComment ? `. Comment: ${lv.adminComment}` : ""
                                }`
                    ...
                    if (status === "Approved" && lv.startDate) {
                        ...
                        if (diffDays >= 0 && diffDays <= 3) {
                            out.push({
                                ...
                                message: isAdmin
                                    ? `Reminder: ${lv.name}'s ${lv.leaveType} starting ${fmtRange(lv.startDate)}.`
                                    : `Your approved ${lv.leaveType || "leave"} starting ${fmtRange(lv.startDate)} is approaching.`,
                                category: "Leave",
                ...
                <div className="rounded-2xl border bg-card p-4 sm:p-6 shadow-sm flex flex-col h-full">
                    ...
                    <div className="mt-5 flex-1 overflow-y-auto">
                        <Tabs value={tab} onValueChange={setTab} className="flex flex-col h-full">
                            ...
                            {statusTabs.map((s) => {
                                const list = filtered.filter((n) => (s === "All" ? true : n.status === s));
                                return (
                                    <TabsContent key={s} value={s} className="mt-4 space-y-3">
                                        {list.length === 0 ? (
                                            ...
                                            list.map((n) => (
                                                <NotificationItem key={n.id} n={n} onView={handleView} />
                                            ))
                                        )}
                                    </TabsContent>
                                );
